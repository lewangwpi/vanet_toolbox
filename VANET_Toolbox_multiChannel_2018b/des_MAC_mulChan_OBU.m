classdef des_MAC_mulChan_OBU< matlab.DiscreteEventSystem & matlab.system.mixin.Propagates & matlab.system.mixin.SampleTime      
...
%         channelNumBuffer = 0; % store channelNumber of payload from APP layer
        multiChannelSwitch = 0;
        SCHidx = 0;
        multiChannelStatus = zeros(1,3);
        % multiChannelStatus 
%             [0,0]-CCH; [1,0]-GI(CCH); [0,1]-SCH; [1,1]-GI(SCH)
    end
...
%           In EDCA, EIFS - DIFS + AIFS[AC] = SIFS+ACKTxTime+AIFS[AC];
        ...
        function [storageSpec,I,O]=getEntityStorageImpl(obj)
            ...
            mgmStorage = obj.queueFIFO('mgmFrame',inf);     % 11 - Management Frame storage
            storageSpec=[payloadStorage,frameStorage,AC0,AC1,AC2,AC3,HCF,Txwaveform,Rxwaveform,TxFrame,mgmStorage];            
            I=[1 9 11];
            O=[10 8];            
        end
...
%             coder.extrinsic('phy_multiChannelSensing');
            coder.extrinsic('phy_multiChannelSensing');
            coder.extrinsic('num2str');
            events=obj.initEventArray;
            hcfCS=0;
            hcfCS=phy_multiChannelSensing(obj.ChanNum,1,0,0); % Channel sensing
            
            isChannelValid = obj.channelValidCheck(hcfCS, obj.multiChannelSwitch);
            
            tag=tag(tag~=0);
            if isChannelValid  %Channel idle                
                switch tag
                    case 'AC0Listen'
...
%                         entity.data.Retry=1;
        ...
        function multichannelswitch = multiChannelCheck(obj, mulChanStatus)
            coder.extrinsic('num2str');
            
            if obj.ChanNum == 0  % if CCH MAC
                if sum(mulChanStatus(1:2)) == 0
                    multichannelswitch = 1;
                else
                    multichannelswitch = 0;
                end
%                 switch sum(mulChanStatus(1:2))
%                     case 0 % CCHI [0,0,0] or CCHIext [0,0,1]
%                         multichannelswitch = 1; % switch is on
%                     case 2 % SCHI(GI) [1,1,x]
%                         if obj.SCHidx == 0  % if no SCH request
%                             disp(['T = ' num2str(obj.getCurrentTime()) ' MAC_' num2str(obj.ChanNum) '_SCHidx : ' num2str(obj.SCHidx) '  ****************No SCH service detected, stay on CCH*********************']);
%                             multichannelswitch = 1;
%                         else                % if SCH is requested
%                             multichannelswitch = 0;
%                         end
%                         
%                     otherwise
%                         multichannelswitch = 0;
%                 end
            else % if SCH MAC, consider mgmFrame
                if mulChanStatus(2) - mulChanStatus(1) == 1 % if [0,1], SCH interval
                    if obj.SCHidx == obj.ChanNum % obj.SCHidx obtained from mgmFrames
                        multichannelswitch = 1;
                    else
                        multichannelswitch = 0;
                    end
    ...
    methods
        function events=setupEvents(obj)            
            ...
            events=obj.initEventArray;
            phy_multiChannelSensing(obj.ChanNum,0,0,0);
            phy_multiChannelSensing(obj.ChanNum,2,0,0);
...
%             events = [events obj.eventGenerate(11,'initTIentity',0,300)];
        end
       ...
       %% matlab.DiscreteEventSystem - action methods
        ...
        function [entity,events] = payloadEntry(obj,~,entity,storage)
%             storage.index
            coder.extrinsic('num2str');
...
%             if obj.isGI
%                 CHInt = 'Guard Interval';
%             else 
%                 if obj.channelInterval
%                     CHInt = 'SCH';
%                 else 
%                     CHInt = 'CCH';
%                 end
%             end
%             disp(['payload enters MAC ' num2str(obj.txAddr) '. Channel interval-> ' CHInt ' sent to channel ' num2str(obj.ChanNum)] );
            ...
            obj.dstAddress=entity.data.dstAddress; 
%             obj.channelNumBuffer = entity.data.channelNum;     
        ...
        function [entity,events]=payloadGenerate(obj,storage,entity,tag)
            ...
            switch tag               
                case 'RxPayload'
                    ...
                    entity.data.channelNum = obj.ChanNum;
...
%             disp(['Vehicle ' num2str(obj.txAddr) 'receive waveform from channel==>' num2str(obj.ChanNum)]);
           if obj.multiChannelSwitch
                if airPropDelay~=0
...
%                     disp('receive waveform correctly');
...
%                     entity.data.channelNum = obj.channelNumBuffer;
                    events=obj.eventForward('output',2,0);
        ...
        function events=waveformExit(obj,storage,entity,~)
            ...
            if storage ==8
                if entity.data.Control==8
                    %test purpose 
                    if obj.ChanNum == 1
                        disp(['T = ' num2str(obj.getCurrentTime()*1000) 'ms' ', Vehicle' num2str(obj.txAddr) '_MAC' num2str(obj.ChanNum) ' sends waveform to channel.'])
                    end 
                    if obj.txtEnable==1
        ...
        function [entity,events]=frameTimer(obj,storage,entity,tag)   
            ...
            switch tag                
                ...
                case 'AC0AIFS'                    
                    if isChannelValid  %Channel free
...
%                     disp(['T = ' num2str(obj.getCurrentTime()*1000) 'ms' ',  node' num2str(obj.txAddr) ': Frame' num2str(entity.data.SN) ' backoff .......']);
                    if obj.txtEnable==1
                        ...
                        if obj.backoff_AC0==0 % backoff counter reach to zero, transmit immediatelly
                            ...
                            if isChannelValid  % Channel free, keep backoff 
...
%             events = obj.initEventArray;
%             coder.extrinsic('num2str');
%             if strcmp(tag, 'initTIentity') 
%                 obj.multiChannelStatus(1) = 1;
%                 obj.multiChannelSwitch = obj.multiChannelCheck(obj.multiChannelStatus);
% %                 if obj.multiChannelSwitch 
% %                     disp(['T = ' num2str(obj.getCurrentTime()) ' Car_' num2str(obj.txAddr) ': MAC_' num2str(obj.ChanNum) ' start GI']);
% %                 end
%                 
...
%             switch tag
%                 case 'GI'
%                     obj.multiChannelStatus(1) = 0;
%                     obj.multiChannelSwitch = obj.multiChannelCheck(obj.multiChannelStatus);
%                     if obj.multiChannelStatus(2) == 0 % if CCHI    
%                         
% %                         if obj.multiChannelSwitch 
% %                             disp(['T = ' num2str(obj.getCurrentTime()) ' Car_' num2str(obj.txAddr) ': MAC_' num2str(obj.ChanNum) ' start CCHI after GI']);
% %                         end
%                         
%                         events = obj.eventTimer('CCHI',46/1000); % CCH interval 46ms
%                     else
%                         
% %                         if obj.multiChannelSwitch
% %                             disp(['T = ' num2str(obj.getCurrentTime()) ' Car_' num2str(obj.txAddr) ': MAC_' num2str(obj.ChanNum) ' start SCHI after GI']);                                                
% %                         end
%                         
%                         events = obj.eventTimer('SCHI',46/1000); % SCH interval 46ms
%                     end                                        
%                 case 'CCHI'  
%         
%                     if obj.SCHidx ~= 0 % SCH channel required, enter GI period
%                         obj.multiChannelStatus(1) = 1; 
%                         obj.multiChannelStatus(2) = 1; %[1,1,0] SCH Guard Interval
%                         obj.multiChannelStatus(3) = 0;
%                         obj.multiChannelSwitch = obj.multiChannelCheck(obj.multiChannelStatus);
%                                                 
% %                         if obj.multiChannelSwitch 
% %                             disp(['T = ' num2str(obj.getCurrentTime()) ' Car_' num2str(obj.txAddr) ': MAC_' num2str(obj.ChanNum) ' start GI after CCHI']);
% %                         end
%                         
%                         events = obj.eventTimer('GI',4/1000); 
%                     else               % SCH service not detected, stay in CCH_MAC
%                         obj.multiChannelStatus(1) = 0; 
%                         obj.multiChannelStatus(2) = 0; 
%                         obj.multiChannelStatus(3) = 1; %[0,0,1] CCHI_extension period
%                         obj.multiChannelSwitch = obj.multiChannelCheck(obj.multiChannelStatus);
%                                                 
% %                         if obj.multiChannelSwitch 
% %                             disp(['T = ' num2str(obj.getCurrentTime()) ' Car_' num2str(obj.txAddr) ': MAC_' num2str(obj.ChanNum) ' start CCHIext after CCHI']);
% %                         end
...
%                 case 'CCHIext'
%                     
%                     obj.multiChannelStatus = zeros(1,3);
%                     events = obj.eventTimer('CCHI', 50/1000);
%                                             
% %                     if obj.multiChannelSwitch 
% %                             disp(['T = ' num2str(obj.getCurrentTime()) ' Car_' num2str(obj.txAddr) ': MAC_' num2str(obj.ChanNum) ' start CCHI after CCHIext']);
% %                         
% %                     end
%                 case 'SCHI'    
%                     
...
%                                             
% %                     if obj.multiChannelSwitch 
% %                             disp(['T = ' num2str(obj.getCurrentTime()) ' Car_' num2str(obj.txAddr) ': MAC_' num2str(obj.ChanNum) ' start GI after SCHI']);
% %                     end
% 
%                     events = obj.eventTimer('GI',4/1000); 
%             end
...
%             %% test purpose
%             if obj.txAddr == 0
%                 switch sum(obj.multiChannelStatus)
%                     case 0
%                         disp('CCH');
%                     case 1
%                         if obj.multiChannelStatus(1) == 1
%                             disp('GI(CCH)');
%                         else
%                             disp('SCH');
%                         end
...
%                 
%                 if obj.multiChannelSwitch
%                     disp(['MAC ' num2str(obj.ChanNum) ': Tx YES!']);
%                 else
%                     disp([ 'MAC' num2str(obj.ChanNum) ': Tx NO!']);
%                 end
%             end
%             
%         end
        ...
        function [entity,events] = mgmFrameEntry(obj,storage,entity,source)
            % mgmFrame(4) - entity.data.field1 = selectSCH.
            coder.extrinsic('num2str');
            events = obj.initEventArray;
            obj.SCHidx = entity.data.field1; 
            if obj.SCHidx == obj.ChanNum
                obj.multiChannelSwitch = 1;
            else 
                obj.multiChannelSwitch = 2;
            end
%             if obj.SCHidx == 0
%                 disp(['    Vehicle' num2str(obj.txAddr) '_MAC' num2str(obj.ChanNum) ' receives MGM from WME ^^^^^^ reset SCH to CCH ']);
%             else
%                 if obj.SCHidx == obj.ChanNum
%                     disp(['    Vehicle' num2str(obj.txAddr) '_MAC' num2str(obj.ChanNum) ' receives MGM from WME ~~~~~~ activate SCH ' num2str(obj.SCHidx)]);
%                 end
%             end
            ...
            events = [events obj.eventDestroy()];
%             if (obj.ChanNum == 0)||(obj.ChanNum ~= entity.data.field1)
%                 events = [events obj.eventDestroy()];
%             else
%                 obj.SCHidx = entity.data.field1;                   
%                 disp(['    Vehicle' num2str(obj.txAddr) '_MAC' num2str(obj.ChanNum) ' receives MGM from WME ~~~~~~ activate SCH ' num2str(obj.SCHidx)]);
%             end 
        ...
        function isChannelValid = channelValidCheck(~, channelSensing, multiChannelInterval)
            if channelSensing == 0 && multiChannelInterval == 1
                isChannelValid = 1;
            else 
                isChannelValid = 0;
            end            
