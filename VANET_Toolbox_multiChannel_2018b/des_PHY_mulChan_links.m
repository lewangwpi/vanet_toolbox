classdef des_PHY_mulChan_links < matlab.DiscreteEventSystem & matlab.system.mixin.Propagates & matlab.system.mixin.SampleTime
    ...
    methods
        function [entity,events]=waveformEntry(obj,storage,entity,src)
            ...
            phy_multiChannelSensing(entity.data.channelNum,0,1,0);
            entity.data.SNR=100;                        
            if sum(entity.data.ACKBody)==0             % Data, not ACK                
                waveformPayloadBuffer=entity.data.Body;   
                phy_multiChannelSensing(entity.data.channelNum,3,waveformPayloadBuffer,entity.data.Address2,obj.getCurrentTime()*1000,obj.phyTXTEnable);
                events=obj.eventTimer('dataProp',obj.channelDelay); % Data tx time
            else                                       % ACK
                waveformPayloadBuffer=entity.data.ACKBody; 
                phy_multiChannelSensing(entity.data.channelNum,4,waveformPayloadBuffer,entity.data.Address2,obj.getCurrentTime()*1000,obj.phyTXTEnable);
        ...
        function [entity,events]=waveformTimer(obj,storage,entity,tag)     
            ...
            switch tag
                case 'dataProp'
                    o=phy_multiChannelSensing(entity.data.channelNum,5,obj.getCurrentTime(),entity.data.Address2,entity.data.ACTag);
                    entity.data.Body=o;
                case 'ackProp'
                    o=ones(975,1)*(1i);
                    o=phy_multiChannelSensing(entity.data.channelNum,6,0,entity.data.Address2);
        ...
        function events=waveformExit(obj,storage,entity,dst)
            ...
            fcn_eventsCount('phy');
            coder.extrinsic('phy_multiChannelSensing');
            phy_multiChannelSensing(entity.data.channelNum,0,0,0);          %reset channel status
            phy_multiChannelSensing(entity.data.channelNum,2,0,0);          % reset channel waveform buffer
