classdef des_APP_OBU < matlab.DiscreteEventSystem & matlab.system.mixin.Propagates & matlab.system.mixin.SampleTime  
    %% properties
    properties
...
%         performanceLaneChangingSwitch=1;
        ...
        carStatus=0; % 0, carFollowing; 1, laneChanging;
        reactPeriodTag=0;       
...
%             end
    ...
    methods
        function events=setupEvents(obj)       
            ...
            events=[obj.eventGenerate(1,'startDriving',(obj.vehicleID-1)*0.01+0.000001*obj.vehicleID,300) obj.eventGenerate(1,'BSMgen',0.000001*obj.vehicleID,100);];                                    
        end
...
%                     disp(num2str(entity.data.priority));
                    ...
                    events=obj.eventTimer('laneChangingRequestTimeout',0.005);
...
%                     disp(['T= ' num2str(obj.getCurrentTime()*1000) 'ms: ' 'Vehicle: ' num2str(obj.vehicleID) 'generate BSM--> curPos: ' num2str(obj.curPositionX) ' curSpeed: ' num2str(obj.curSpeedX) ' curAcc: ' num2str(obj.acceleration)]);                    
                ...
                case 'RDTgen'
                    if obj.appTXTEnable && obj.vehicleID==1 
                        disp(['=============> Car' num2str(obj.vehicleID) ' -> generate lane changing REQUEST ===============>>' num2str(obj.dstAddress)]);
                    end
                    payloadlength=evalin('base','payloadLength');
                    entity.data.wsmp=ones(1,payloadlength);
                    entity.data.VehicleID=obj.vehicleID;
                ...
                case 'RDTreply'
                    if obj.appTXTEnable && obj.vehicleID==1 
                        disp(['T=' num2str(obj.getCurrentTime()*1000) 'ms: Car' num2str(obj.vehicleID) '->generate lane changing REPLY.<====']);
                    end
                    payloadlength=evalin('base','payloadLength');
                    entity.data.wsmp=ones(1,payloadlength);
                    entity.data.VehicleID=obj.vehicleID;
...
%                             disp(['Vehicle ' num2str(obj.vehicleID) ' on INTERSECTION mode ++++++ at postion of ' num2str(obj.curPositionX) 'speed: ' num2str(obj.curSpeedX)]);
        ...
        function events=rcvEMG(obj,entity)
            ...
            if posX<=obj.curPositionX % car in front 
                if obj.appTXTEnable && obj.vehicleID==1 
                    disp(['T=' num2str(obj.getCurrentTime()*1000) 'ms. Car' num2str(obj.vehicleID) ': lane changing request received from BACK <<<<<car' num2str(srcAddress) '>'])
                end
                ...
                if obj.appTXTEnable && obj.vehicleID==1 
                    disp(['T=' num2str(obj.getCurrentTime()*1000) 'ms. Car' num2str(obj.vehicleID) ': lane changing request received from FRONT <<<<<car' num2str(srcAddress) '>'])
                end
        ...
        function events=rcvRDT(obj,entity)
            ...
            if laneChangingReply==1
                if obj.appTXTEnable && obj.vehicleID==1 
                    disp(['T=' num2str(obj.getCurrentTime()*1000) 'ms: car' num2str(rcvCarID) ' reply to car' num2str(obj.vehicleID) ': lane changing request permited ^_^ ']);
                end
...
%             disp(['<--Speed-->: vehicle:' num2str(obj.vehicleID)  ' acceleration:' num2str(obj.acceleration) 'speed: ' num2str(obj.curSpeedX)]);                                                   
        ...
        function changeLane(obj)
            ...
            if obj.carStatus==0
                if obj.appTXTEnable && obj.vehicleID==1 
                    disp(['T=' num2str(obj.getCurrentTime()*1000) ': Car' num2str(obj.vehicleID) '^^START^^LANE^^CHANGING^^']);                                            
                end
                                                                
                LCLatency=obj.getCurrentTime()*1000-obj.LCtimeTag;
                
                disp(['Car' num2str(obj.vehicleID) ': Current Time: ' num2str(obj.getCurrentTime()*1000) 'ms - LCtimeTag ' num2str(obj.LCtimeTag) 'ms = LCLatency' num2str(LCLatency) 'ms']);
